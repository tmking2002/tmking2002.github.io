...
```{=html}
<img src="headshots/valerie_cagle.png" alt="headshot" class="center" style="width:150px;">
```
...
```{r cagle_spin}
plot <- ggplot(cagle %>% filter(PitchType %in% cagle_pitch_metrics$PitchType), aes(x = adj_SpinAxis, y = RelSpeed)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
...
ggsave("cagle_spin.png", plot, bg = "transparent")
```

```{=html}
<img src="cagle_spin.png" alt="spin_profile" class="center" style="width:650px;">
```