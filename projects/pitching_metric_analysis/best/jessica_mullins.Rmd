...
```{=html}
<img src="headshots/jessica_mullins.jpg" alt="headshot" class="center" style="width:150px;">
```
...
```{r mullins_spin, include=FALSE}
plot <- ggplot(mullins %>% filter(PitchType %in% mullins_pitch_metrics$PitchType), aes(x = adj_SpinAxis, y = RelSpeed)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
...
ggsave("mullins_spin.png", plot, bg = "transparent")
```

```{=html}
<img src="mullins_spin.png" alt="spin_profile" class="center" style="width:650px;">
```
