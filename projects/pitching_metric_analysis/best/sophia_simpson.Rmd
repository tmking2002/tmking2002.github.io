...
```{=html}
<img src="headshots/sophia_simpson.jpg" alt="headshot" class="center" style="width:150px;">
```
...
```{r simpson_spin}
plot <- ggplot(simpson %>% filter(PitchType %in% simpson_pitch_metrics$PitchType), aes(x = adj_SpinAxis, y = RelSpeed)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
...
ggsave("simpson_spin.png", plot, bg = "transparent")
```

```{=html}
<img src="simpson_spin.png" alt="spin_profile" class="center" style="width:650px;">
```