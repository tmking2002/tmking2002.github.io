...
```{=html}
<img src="headshots/rachel_riley.jpg" alt="headshot" class="center" style="width:150px;">
```


```{r riley_data}
riley <- combined_data %>% 
  filter(Pitcher == "Rachel Riley" & adj_SpinAxis > 150 & adj_SpinAxis < 270)
```

```{r riley_pitch_breakdown}
riley_pitch_metrics <- riley %>% 
...
```{r riley_break}
ggplot(riley, aes(x = HorzBreak, y = InducedVertBreak)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
  geom_point(data = riley_pitch_metrics, aes(x = HorzBreak, y = InducedVertBreak, color = PitchType), size = 5) +
  xlim(-5,5) + 
  ylim(-5,5) + 
  theme_ipsum_es() +
...
```{r riley_spin}
plot <- ggplot(riley %>% filter(PitchType %in% riley_pitch_metrics$PitchType), aes(x = adj_SpinAxis, y = RelSpeed)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
  geom_point(data = riley_pitch_metrics, aes(x = SpinAxis, y = RelSpeed, color = PitchType), size = 5) +
  coord_polar() +
...
ggsave("riley_spin.png", plot, bg = "transparent")
```

```{=html}
<img src="riley_spin.png" alt="spin_profile" class="center" style="width:650px;">
```