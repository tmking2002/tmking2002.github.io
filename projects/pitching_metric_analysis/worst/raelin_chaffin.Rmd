...
```{=html}
<img src="headshots/raelin_chaffin.png" alt="headshot" class="center" style="width:150px;">
```


```{r chaffin_data}
chaffin <- combined_data %>% 
  filter(Pitcher == "Raelin Chaffin")
```

```{r chaffin_pitch_breakdown}
chaffin_pitch_metrics <- chaffin %>% 
...
```{r chaffin_break}
ggplot(chaffin, aes(x = HorzBreak, y = InducedVertBreak)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
  geom_point(data = chaffin_pitch_metrics, aes(x = HorzBreak, y = InducedVertBreak, color = PitchType), size = 5) +
  xlim(-10,0) + 
  ylim(-5,5) + 
  theme_ipsum_es() +
...
```{r chaffin_spin}
plot <- ggplot(chaffin %>% filter(PitchType %in% chaffin_pitch_metrics$PitchType), aes(x = adj_SpinAxis, y = RelSpeed)) +
  geom_point(aes(color = PitchType), alpha = .7) + 
  geom_point(data = chaffin_pitch_metrics, aes(x = SpinAxis, y = RelSpeed, color = PitchType), size = 5) +
  coord_polar() +
...
ggsave("chaffin_spin.png", plot, bg = "transparent")
```

```{=html}
<img src="chaffin_spin.png" alt="spin_profile" class="center" style="width:650px;">
```